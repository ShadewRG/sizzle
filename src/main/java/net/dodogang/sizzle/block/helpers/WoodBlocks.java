package net.dodogang.sizzle.block.helpers;

import static net.dodogang.sizzle.init.SBlocks.*;
import static net.dodogang.sizzle.init.SEntities.*;
import static net.dodogang.sizzle.init.SItems.*;
...
public class WoodBlocks {
    ...
    public EntityType<SBoatEntity> BOAT;
    public SBoatItem BOAT_ITEM;
    ...
    public WoodBlocks(WoodBlocksConfig config) {
        ...
        TRAPDOOR = new PublicTrapdoorBlock(AbstractBlock.Settings.of(config.woodMaterial, config.woodMaterialColor).strength(3.0F).sounds(BlockSoundGroup.WOOD).nonOpaque().allowsSpawning(SBlocks::never));
        ...
        Identifier signTexture = new Identifier(Sizzle.MOD_ID, "entity/signs/" + id);
        SIGN = new SSignBlock(signTexture, AbstractBlock.Settings.of(Material.WOOD).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD));
        WALL_SIGN = new SWallSignBlock(signTexture, AbstractBlock.Settings.of(Material.WOOD).noCollision().strength(1.0F).sounds(BlockSoundGroup.WOOD).dropsLike(SIGN));
        ...
        LOG = register(config.id + "_" + config.log_suffix, LOG);
        STRIPPED_LOG = register("stripped_" + config.id + "_" + config.log_suffix, STRIPPED_LOG);

        WOOD = register(config.id + "_" + config.wood_suffix, WOOD);
        STRIPPED_WOOD = register("stripped_" + config.id + "_" + config.wood_suffix, STRIPPED_WOOD);
        ...
        SIGN_ITEM = register(config.id, (SignBlock)SIGN, (WallSignBlock)WALL_SIGN);

        if (BOAT != null) BOAT = register(this, config.vanillaBoatType);
        if (BOAT_ITEM != null) BOAT_ITEM = register(this);
    ...
    public void clientRegistries() {
        ...
        // signs
        SpriteIdentifierRegistry.INSTANCE.addIdentifier(new SpriteIdentifier(TexturedRenderLayers.SIGNS_ATLAS_TEXTURE, new Identifier(Sizzle.MOD_ID, "entity/signs/" + id)));
    }
